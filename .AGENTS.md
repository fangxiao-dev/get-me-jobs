...
```text
Apify Task -> raw JSON -> preference filter -> selected JSON -> human annotations -> accepted-job registry -> application dashboard -> preference analysis -> confirmed preference update
```
...
- Treat `data/accepted-jobs.json` and `data/applications.json` as runtime tracking data.
...
- `annotations/*.json`: human review labels.
