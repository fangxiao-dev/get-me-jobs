...
test("loadJobSourcesManifest reads the tracked default manifest without secrets", () => {
  ...
  assert.equal(manifest.channels.apify_linkedin.taskEnvPrefix, "TASKID_");
  assert.equal(manifest.channels.localLinkedin.enabled, true);
...
test("loadJobSourcesManifest rejects invalid manifests", () => {
  ...
  assert.throws(
    ...
    /channels\.apify_linkedin\.enabled must be a boolean/,
  );
});
